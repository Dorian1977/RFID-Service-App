...
namespace BlowFishClassLibrary {
	...
	void BlowFish::Blowfish_encipher(DWORD* xl, DWORD* xr, DWORD* PArray, DWORD(*SBoxes)[256])
	{
		union aword  Xl, Xr;
	...
	void BlowFish::Blowfish_decipher(DWORD* xl, DWORD* xr, DWORD* PArray, DWORD(*SBoxes)[256])
	{
		union aword  Xl;
		union aword  Xr;
	...
	void BlowFish::Initialize(BYTE* key, int keybytes, DWORD* PArray, DWORD(*SBoxes)[256])
	{
		int  		i, j;
		...
		{
			Blowfish_encipher(&datal, &datar, PArray, SBoxes);
			PArray[i] = datal;
			PArray[i + 1] = datar;
		}
	...
	DWORD BlowFish::Encode(BYTE key[], int keybytes, BYTE pInput[], BYTE* pOutput, DWORD lSize)
	{
		DWORD 	lCount, lOutSize, lGoodBytes;
		BYTE* pi, * po;
		int		i, j;
		int		SameDest = (pInput == pOutput ? 1 : 0);
		DWORD* PArray = new DWORD[18];
		DWORD(*SBoxes)[256] = new DWORD[4][256];

		Initialize(key, keybytes, PArray, SBoxes);
	...
	// output buffer can be the same, but be sure buffer length is even MOD 8.
	void BlowFish::Decode(BYTE key[], int keybytes, BYTE pInput[], BYTE* pOutput, DWORD lSize)
	{
		DWORD 	lCount;
		BYTE* pi, * po;
		int		i;
